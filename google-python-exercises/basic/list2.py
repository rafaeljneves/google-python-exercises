...
def remove_adjacent(nums):
    for i,number in zip(range(0,len(nums)-1),nums):
        if i == len(nums)-1:
            if nums[i] == nums[i-1]:
                del nums[i]
            return nums
...
def linear_merge(list1, list2):
      ...
      lista_ordenada = list1 + list2
      for i,number in zip(range(0,len(lista_ordenada)-1),lista_ordenada):
            if i == len(lista_ordenada)-1:
                if lista_ordenada[i] < lista_ordenada[i-1]:
                    aux = lista_ordenada[i-1]
